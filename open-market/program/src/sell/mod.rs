...
use crate::{constants::*, errors::*, id, sell::config::*};
...
pub struct AuctioneerSell<'info> {
    ...
    #[account(mut, seeds=[PREFIX.as_bytes(), wallet.key().as_ref(), id().as_ref(), token_account.key().as_ref(), auction_house.treasury_mint.as_ref(), token_account.mint.as_ref(), &u64::MAX.to_le_bytes(), &token_size.to_le_bytes()], seeds::program=auction_house_program, bump=trade_state_bump)]
    pub seller_trade_state: UncheckedAccount<'info>,

    /// CHECK: Not dangerous. Account seeds checked in constraint.
    /// Free seller trade state PDA account encoding a free sell order.
    #[account(mut, seeds=[PREFIX.as_bytes(), wallet.key().as_ref(), id().as_ref(), token_account.key().as_ref(), auction_house.treasury_mint.as_ref(), token_account.mint.as_ref(), &0u64.to_le_bytes(), &token_size.to_le_bytes()], seeds::program=auction_house_program, bump=free_trade_state_bump)]
...
pub fn auctioneer_sell<'info>(
    ...
    ctx.accounts.listing_config.listing_auction_house =
        ctx.accounts.auction_house.to_account_info().key();
    ctx.accounts.listing_config.listing_fee = LISTING_FEE;
