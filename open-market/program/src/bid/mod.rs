...
pub struct AuctioneerBuy<'info> {
    ...
    #[account(mut, seeds = [PREFIX.as_bytes(), wallet.key().as_ref(), id().as_ref(), token_account.key().as_ref(), treasury_mint.key().as_ref(), token_account.mint.as_ref(), buyer_price.to_le_bytes().as_ref(), token_size.to_le_bytes().as_ref()], seeds::program=auction_house_program, bump = trade_state_bump)]
